...
// this HAL supports ESP32, ESP32S2, ESP32S3, ESP32C3, and other ESP32 variants
// Updated to support ESP32S3 and other targets
...
class EspHal : public RadioLibHal {
  public:
    ...
    void spiBegin() {
      ...
      // Set initial states
      this->digitalWrite(this->spiSCK, LOW);
      this->digitalWrite(this->spiMOSI, LOW);
    }
    ...
    uint8_t spiTransferByte(uint8_t b) {
      // Improved bit-bang SPI for better reliability
      uint8_t result = 0;
      ...
      for(int i = 7; i >= 0; i--) {
        ...
        this->digitalWrite(this->spiMOSI, (b >> i) & 0x01);
        this->delayMicroseconds(2);
        
        // Clock high - data is captured on rising edge
        this->digitalWrite(this->spiSCK, HIGH);
        this->delayMicroseconds(2);
        
        // Read MISO bit
        if(this->digitalRead(this->spiMISO)) {
          result |= (1 << i);
        }
